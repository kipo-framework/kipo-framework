...
def hello(name: str = "Kipo User"):
    ...
    """
    console.print(Panel.fit(f"[bold green]Hello {name}! Welcome to Kipo ETL Framework![/bold green]", title="Kipo System"))
...
@app.command()
def init(
    project_name: Optional[str] = typer.Argument(None, help="Name of the project directory")
):
    """
...
def show(
    ...
    try:
        ...
        try:
            ...
            console.print(f"[bold red]❌ Invalid Layer:[/bold red] '{layer}'. Options are: bronze, silver, gold.")
            raise typer.Exit(code=1)

        # 2. Feedback visual
        console.print(f"[bold blue]🔍 Inspecting:[/bold blue] {name} [{target_layer}]")
